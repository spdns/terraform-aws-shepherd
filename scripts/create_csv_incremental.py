...
REQUIRED_PARAMS = ["region", "athenaDatabase", "athenaTable", "inputBucket",
    "salt", "ordinal", "subscriber", "receiver", "JOB_NAME"]
OPTIONAL_PARAMS = ["dayRange", "maxHoursAgo", "fullDays", "hourly", "parentPolicies", "policies",
    "delimiter", "outputBucket", "inputPrefix", "outputDir", "outputFilename",
    "dontPreserveOutputDir", "nonStrict", "keepOrigOnRename", "verbose"]
PARAM_START_INDEX = 1
FN_INDEX = 0
DT_INDEX = 1
START_TIME = int(time())
...
def validate_db(database, create_db=False, glue_client=None, region_name=None):
    ...
    db_resp = 0
    try:
        db_resp = glue_client.get_database(
            Name=database)[B.RESP_META][B.HTTP_STATUS]
    except ClientError as ce:
        db_resp = ce.response[B.RESP_META][B.HTTP_STATUS]
    
    # Only acceptable outcome: 200 (database exists and we can access it)
    if not db_resp == 200:
        raise ValidationException("Could not verify database %s exists: %s" %(
            database, db_resp))
...
def validate_table(database, table, glue_client=None, region_name=None):
    ...
    try:
        table_resp = glue_client.get_table(
            ...
            Name=table)[B.RESP_META][B.HTTP_STATUS]
    except ClientError as ce:
    ...
    if not table_resp == 200:
        ...
        if table_resp == 400:
            raise ValidationException("Table/view %s.%s could not be found: %s" %(database, table, table_resp))
        elif table_resp == 403:
            raise ValidationException("Table/view %s.%s is not authorized: %s" %(database, table, table_resp))
        else:
            raise ValidationException("Received unacceptable HTTP response code for table/view " \
...
def validate_bucket(bucket, s3_client=None, region_name=None):
    ...
    http_resp = 0
    try:
        http_resp = s3_client.head_bucket(Bucket=bucket).get(B.RESP_META, {}).get(B.HTTP_STATUS, 0)
    # Catch non-200 responses.
    except ClientError as ce:
        http_resp = int(ce.response[B.ERROR][B.CODE])
    
    if not http_resp == 200:
        raise ValidationException("Could not verify bucket s3://%s exists and is accessible: %s" %(
                bucket, http_resp))
...
def get_latest_file(bucket, prefix=None, s3_client=None, region_name=None):
    ...
    for chunk in paginator.paginate(Bucket=bucket, Prefix=prefix):
        files = [(fn.get("Key", ""), fn.get("LastModified", 0)) for fn in chunk.get("Contents", [])
...
def rename_file(bucket, rename_fn, prefix=None, dont_preserve_dir=True, keep_orig=False, 
    s3_client=None, region_name=None):
    if not s3_client:
        s3_client = boto3.client("s3", region_name=region_name)
    ...
    if not candidate_fn:
        raise Exception("Unable to find job output file in s3://%s/%s" %(bucket, prefix))
    ...
    copy_resp = s3_resource.Object(
        bucket, "%s%s" %(rename_dir, rename_fn)).copy_from(
...
def get_table_schema(database, table, glue_client=None, region_name=None):
    ...
    return [(col.get(B.NAME), col.get(B.TYPE)) for col in cols] + [(p_col.get(B.NAME), p_col.get(B.TYPE)) for p_col in p_cols]
...
def get_args():
    ...
    param_pairs = dict([raw_params[index:index+2] for index in range(0, len(raw_params), 2)])
    ...
    args["policyStrings"] = args.get("policies") if args.get("policies") else args.get("parentPolicies")
    ...
    if args.get("maxHoursAgo") is not None:
        try:
            ...
            raise ValidationException("If set, --maxHoursAgo must be an integer. Value received %s" %(args["maxHoursAgo"]))
        if args["maxHoursAgo"] < 0:
            raise ValidationException("--maxHoursAgo cannot be under 0.")
        elif not args["maxHoursAgo"]:
            print("WARNING: maxHoursAgo set to 0. View will read only data for the current hourly partition.")

    args = SimpleNamespace(**args)

    # Validate we received only bucket names.
    if args.outputBucket.lower().startswith("s3://") or args.inputBucket.lower().startswith("s3://"):
        raise ValidationException("Bucket params must be bucket names only (i.e., not start with s3://).")
    elif args.outputBucket.endswith('/') or args.inputBucket.endswith('/'):
        raise ValidationException("Bucket params must not end with trailing slash (i.e., / ).")
...
def main(args):
    ...
    if validate_table(args.athenaDatabase, args.athenaTable, glue_client=glue_client) and args.verbose:
    ...
    input_csv, latest_dt = get_latest_file(args.inputBucket, prefix=args.inputPrefix, s3_client=s3_client)    
    
    if not input_csv:
        raise Exception("Found no candidate CSV files in bucket %s with prefix %s." %(
            args.inputBucket, "(no prefix)" if not args.inputPrefix else args.inputPrefix))
            
    print("Got latest CSV file s3://%s/%s with write time %s." %(args.inputBucket, input_csv, latest_dt))
    ...
    prelim_min_hour = (((START_TIME - (3600 * args.maxHoursAgo)) // aligner ) * aligner) * 1000000
    
    # Get CSV file contents, filtering out lines that don't fall within --maxHoursAgo.
    input_df = gc.create_dynamic_frame_from_options("s3", 
        {'paths': ["s3://%s/%s" %(args.inputBucket, input_csv)]},
        "csv",
        {"withHeader": True}).toDF().select("*")
    print("Input DF count: %s" %(input_df.count()))
    csv_prelim_df = input_df.filter("cast(start_time as bigint) >= %s" %(prelim_min_hour))
    ...
    if args.nonStrict:
        ...
        last_seen_epoch = input_df.selectExpr('cast(start_time as bigint) as ts').agg({"start_time": "max"}).collect().pop()[0]
    ...
    csv_hits_in_range = csv_prelim_df.filter("cast(start_time as bigint) < %s" %(increm_min_hour_micro))
    ...
    tmp_schema = get_table_schema(args.athenaDatabase, args.athenaTable, glue_client=glue_client)
    # Verify we only have one matching column on which to explode.
    if not [col[0] for col in tmp_schema].count(args.policyType) == 1:
        raise Exception("Wrong number of matching %s columns found in schema: %s" %(args.policyType, tmp_schema))
    ...
    pol_arr = ", ".join(["'%s'" % (pol) for pol in args.policyStrings.split(args.delimiter) if pol])
    ...
    if args.outputFilename:
        rename_resp = rename_file(args.outputBucket, 
            args.outputFilename, 
